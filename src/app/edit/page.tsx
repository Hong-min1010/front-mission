...
export default function EditPage() {
  ...
  return (
    <div className="flex flex-row h-screen bg-gray-700 text-white">
      ...
      <main className="flex flex-col w-full relative p-5 items-center">
              ...
              <div className="flex flex-row gap-3 my-2">
                {(['NOTICE', 'FREE', 'QNA', 'ETC'] as CategoryKey[]).map((key) => (
                  <button
                    ...
                    className={`cursor-pointer font-bold px-3 py-1 rounded-lg border border-black text-gray-700 text-xl bg-white hover:bg-gray-200 transition ${
                      category === key ? `${CATEGORY_COLOR[key]} ring-2 ring-black` : ''
                    }`}
                  >
                    {CATEGORY_LABEL[key]}
                  </button>
                ...
                {(previewUrl || detail.imageUrl) && (
                  <div className="w-full max-w-[400px]">
                    <img
                      ...
                      alt="미리보기"
                      className="w-full h-auto rounded-lg bg-white mt-2"
                    />
                  </div>
                )}
